...
const Dashboard = () => {
  const { getDashData, aToken, dashData, cancleAppointment } = useContext(AdminContext);
    ...
    <div className='m-5'>
      <div className='flex flex-wrap gap-3'>
        ...
        <div className='flex items-center gap-2 bg-white p-4 min-w-52 rounded border-2 border-gray-200 cursor-pointer hover:scale-105 transition-all'>
          ...
          <div>
            <p className='text-xl font-semibold text-gray-600'>{dashData.paitents}</p>
            <p className='text-gray-400'>Patients</p>
          </div>
        </div>
      </div>
      <div className='bg-white'>
        <div className='flex items-center gap-2.5 px-4 py-5 mt-10 rounded-t border'>
              ...
              dashData.latestAppointments.map((item, index) => (
                <div className='flex items-center px-6 py-3 gap-3' key={index}>
                  ...
                  {item.canclled ? (
                    ...
                    <img onClick={() => cancleAppointment(item._id)} className='w-8 cursor-pointer' src={assets.cancel_icon} alt='Cancel' />
                  )}
                </div>
              ))
            ) : (
              <p className='px-6 py-4 text-gray-500'>No recent appointments found.</p>
            )
          }
